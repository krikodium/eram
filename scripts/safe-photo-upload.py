#!/usr/bin/env python3
"""
Script de Subida Segura de Fotos
Sube solo las fotos faltantes sin duplicar ni sobrescribir
ESTRATEGIA CONSERVADORA - Solo sube lo que falta
"""
...
def upload_photos_safely(drive_service, supabase, photos_to_upload, dry_run=True):
    ...
    for i, photo in enumerate(photos_to_upload, 1):
        ...
        print(f"\n[{i}/{len(photos_to_upload)}] Procesando {code} ({name})")
...
def main():
    """Función principal de subida segura."""
    print("SUBIDA SEGURA DE FOTOS - ERAM")
    print("=" * 60)
    ...
    try:
        ...
        print(f"\n¿Que quieres hacer?")
        print("1. Simular subida (dry-run)")
        print("2. Subir solo prioridad alta")
        print("3. Subir todas las fotos")
        print("4. Salir")
        ...
        if choice == "1":
            # Simular subida
            upload_photos_safely(drive_service, supabase, priority_photos + other_photos, dry_run=True)
        elif choice == "2":
            # Subir solo prioridad alta
            upload_photos_safely(drive_service, supabase, priority_photos, dry_run=False)
        elif choice == "3":
            # Subir todas
            upload_photos_safely(drive_service, supabase, priority_photos + other_photos, dry_run=False)
        elif choice == "4":
            print("Saliendo...")
            return
        else:
