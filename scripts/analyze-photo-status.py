#!/usr/bin/env python3
"""
Script de Análisis Completo de Fotos
...
def analyze_drive_photos(drive_service, folder_ids):
    ...
    for i, folder_id in enumerate(folder_ids, 1):
        folder_name = f"Carpeta {i}" if i <= 5 else "Producción 11/08"
...
def analyze_database_products(supabase):
    ...
    try:
        ...
        if len(product_codes) != len(unique_codes):
            ...
            print(f"⚠️  Códigos duplicados en BD: {duplicates}")
...
def analyze_duplicates_and_matches(drive_photos, supabase_files, product_codes):
    ...
    print(f"📊 Códigos únicos en Drive: {len(set(drive_codes))}")
    print(f"📊 Total archivos en Drive: {len(drive_codes)}")
    print(f"🔄 Códigos duplicados en Drive: {len(drive_duplicate_codes)}")
    ...
    if drive_duplicate_codes:
        ...
        if len(drive_duplicate_codes) > 10:
            print(f"      ... y {len(drive_duplicate_codes) - 10} códigos más con duplicados")
    ...
    print(f"\n🎯 COINCIDENCIAS:")
    print(f"   ✅ Códigos que coinciden (Drive ↔ BD): {len(matches)}")
    print(f"   📁 Solo en Drive: {len(drive_only)}")
    print(f"   🗄️  Solo en BD: {len(bd_only)}")
    ...
    if drive_only:
        print(f"\n📁 CÓDIGOS SOLO EN DRIVE (sin producto en BD):")
        for code in list(drive_only)[:10]:
            print(f"   {code}")
        if len(drive_only) > 10:
            print(f"   ... y {len(drive_only) - 10} más")
    
    # Mostrar códigos solo en BD
    if bd_only:
        print(f"\n🗄️  CÓDIGOS SOLO EN BD (sin foto en Drive):")
        for code in list(bd_only)[:10]:
            print(f"   {code}")
        if len(bd_only) > 10:
            print(f"   ... y {len(bd_only) - 10} más")
...
def generate_recommendations(analysis_results, without_image, drive_duplicates):
    ...
    print(f"   Acción: Subir fotos faltantes")
    ...
    if products_needing_photos:
        ...
        if len(products_needing_photos) > 10:
            print(f"      ... y {len(products_needing_photos) - 10} más")
    
    # Duplicados en Drive
    print(f"\n🔄 DUPLICADOS EN DRIVE:")
    print(f"   Cantidad de códigos con duplicados: {len(drive_duplicates)}")
    print(f"   Acción: Revisar manualmente o elegir la mejor foto")
    
    # Códigos solo en Drive
    print(f"\n📁 CÓDIGOS SOLO EN DRIVE:")
    print(f"   Cantidad: {len(drive_only)}")
    print(f"   Acción: Verificar si son productos válidos o archivos basura")
    
    # Códigos solo en BD
    print(f"\n🗄️  CÓDIGOS SOLO EN BD:")
    print(f"   Cantidad: {len(bd_only)}")
    print(f"   Acción: Buscar fotos faltantes o marcar como sin imagen")
...
def main():
    ...
    try:
        ...
        print(f"\n📊 RESUMEN FINAL:")
        print("=" * 50)
        print(f"🖼️  Fotos en Drive: {len(drive_photos)}")
        print(f"☁️  Fotos en Supabase: {len(supabase_files)}")
        print(f"🗄️  Productos en BD: {len(products)}")
        print(f"✅ Productos con imagen: {len(with_image)}")
        print(f"❌ Productos sin imagen: {len(without_image)}")
        print(f"🎯 Coincidencias Drive↔BD: {len(analysis_results['matches'])}")
        print(f"🔄 Códigos duplicados en Drive: {len(analysis_results['drive_duplicates'])}")
